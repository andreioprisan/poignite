...
//		$_SESSION['currentpo']['step1']['author_email'] = $this->userdata['email'];
	...
	public function newpo()
	{
		...
		if ($step == "1")
		{
			...
			$payload['step'] = $step;
			if (isset($_SESSION['currentpo']['step4']['pouniqueid']))
			{
				$this->pouid = $_SESSION['currentpo']['step4']['pouniqueid'];
				$this->poid = $this->posm->getpoid($this->pouid);
				$files = $this->filesm->getpofiles($this->poid);
				$payload['pouid'] = $this->pouid;
				$payload['poid'] = $this->poid;
			}
			$payload['files'] = $files;
	...
	public function viewpo($pouid)
	{
		$currentpo = $this->posm->getpouid($pouid);
		$this->pouid = $pouid;
		$this->poid = $this->posm->getpoid($pouid);
		$this_po = get_object_vars(json_decode($currentpo->content));
